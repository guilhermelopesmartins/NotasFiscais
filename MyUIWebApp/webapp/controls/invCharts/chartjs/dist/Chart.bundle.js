...
(function(global, factory) {
  ...
  var helpers_options = {
    ...
    toLineHeight: function(value, size) {
      var matches = (String(value)).match(/^(normal|(\d+(?:\.\d+)?)(px|em|%)?)$/);
    ...
    toPadding: function(value) {
      ...
      if (helpers_core.isObject(value)) {
        t = Number(value.top) || 0;
        r = Number(value.right) || 0;
        b = Number(value.bottom) || 0;
        l = Number(value.left) || 0;
      } else {
        t = r = b = l = Number(value) || 0;
      }
  ...
  var core_animations = {
    ...
    advance: function() {
      ...
      while (i < animations.length) {
        ...
        nextStep =
          Math.floor((Date.now() - animation.startTime) / animation.duration * numSteps) +
          1;
  ...
  var controller_bar = core_datasetController.extend({
    ...
    calculateBarValuePixels: function(datasetIndex, index) {
      ...
      var value = Number(scale.getRightValue(datasets[datasetIndex].data[index]));
      ...
      if (stacked || (stacked === undefined && stack !== undefined)) {
        for (i = 0; i < datasetIndex; ++i) {
          ...
          if (
            ...
            ivalue = Number(scale.getRightValue(datasets[i].data[index]));
  ...
  function watchForRender(node, handler) {
    ...
    expando.reflow = Boolean(node.offsetParent);
  ...
  function createTooltipItem(element) {
    ...
    return {
      ...
      label: indexScale ? String(indexScale.getLabelForIndex(index, datasetIndex)) : '',
      value: valueScale ? String(valueScale.getLabelForIndex(index, datasetIndex)) : '',
      index: index,
  ...
  var core_ticks = {
    ...
    formatters: {
      ...
      values: function(value) {
        return helpers$1.isArray(value) ? value : String(value);
      },
  ...
  var core_scale = core_element.extend({
    ...
    afterBuildTicks: function(ticks) {
      ...
      me.ticks =
        helpers$1.callback(me.options.afterBuildTicks, [me, me.ticks]) || me.ticks;
      return ticks;
    },
        ...
        if (optionTicks.display) {
          ...
          if (helpers$1.isArray(label)) {
            for (var i = 0; i < label.length; ++i) {
              ...
              context.fillText(String(label[i]), 0, y);
              y += lineHeight;
            }
          } else {
  ...
  var scale_linear = scale_linearbase.extend({
        ...
        helpers$1.each(datasets, function(dataset, datasetIndex) {
          ...
          if (chart.isDatasetVisible(datasetIndex) && IDMatches(meta)) {
            helpers$1.each(dataset.data, function(rawValue, index) {
              var value = Number(me.getRightValue(rawValue));
    ...
    getLabelForIndex: function(index, datasetIndex) {
      return Number(this.getRightValue(this.chart.data.datasets[datasetIndex].data[index]));
    },
    ...
    getPixelForValue: function(value) {
      ...
      var rightValue = Number(me.getRightValue(value));
      var pixel;
  ...
  var scale_logarithmic = core_scale.extend({
    ...
    getPixelForValue: function(value) {
      ...
      value = Number(me.getRightValue(value));
      if (reverse) {
        start = me.end;
        end = me.start;
        sign = -1;
  ...
  function parse(scale, input) {
    ...
    if (options.round) {
      value = Number(scale._adapter.startOf(value, options.round));
    }
  ...
  function generate(scale, min, max, capacity) {
    ...
    if (weekday) {
      first = Number(adapter.startOf(first, 'isoWeek', weekday));
      last = Number(adapter.startOf(last, 'isoWeek', weekday));
    }

    // Align first/last ticks on unit
    first = Number(adapter.startOf(first, weekday ? 'day' : minor));
    last = Number(adapter.startOf(last, weekday ? 'day' : minor));

    // Make sure that the last tick include max
    if (last < max) {
      last = Number(adapter.add(last, 1, minor));
    }
    ...
    if (majorTicksEnabled && major && !weekday && !timeOpts.round) {
      ...
      time = Number(adapter.startOf(time, major));
      time = +adapter.add(
        time,
    ...
    for (; time < last; time = Number(adapter.add(time, stepSize, minor))) {
  ...
  function ticksFromTimestamps(scale, values, majorUnit) {
    ...
    for (i = 0, ilen = values.length; i < ilen; ++i) {
      value = values[i];
      major = majorUnit ? value === Number(scale._adapter.startOf(value, majorUnit)) : false;
  ...
  var scale_time = core_scale.extend({
    ...
    determineDataLimits: function() {
      ...
      min = min === MAX_INTEGER ? Number(adapter.startOf(Date.now(), unit)) : min;
      max = max === MIN_INTEGER ? Number(adapter.endOf(Date.now(), unit)) + 1 : max;
    ...
    tickFormatFunction: function(time, index, ticks, format) {
      ...
      var majorTime = Number(adapter.startOf(time, majorUnit));
    ...
    getPixelForOffset: function(time) {
      ...
      var offset =
        size * (me._offsets.start + pos) / (me._offsets.start + 1 + me._offsets.end);
  ...
  var moment = createCommonjsModule(function(module, exports) {
    (function(global, factory) {
      ...
      function createDuration(input, key) {
        ...
        } else if (Boolean(match = aspNetRegex.exec(input))) {
          sign = match[1] === '-' ? -1 : 1;
          duration = {
            y: 0,
        ...
        } else if (Boolean(match = isoRegex.exec(input))) {
          sign = match[1] === '-' ? -1 : 1;
          duration = {
      ...
      function endOf(units) {
        ...
        switch (units) {
          ...
          case 'week':
            time =
              startOfDate(this.year(), this.month(), this.date() - this.weekday() + 7) -
              1;
            break;
          case 'isoWeek':
            time =
  ...
  function decodeFill(el, index, count) {
    ...
    if (fill === undefined) {
      fill = Boolean(model.backgroundColor);
    }
