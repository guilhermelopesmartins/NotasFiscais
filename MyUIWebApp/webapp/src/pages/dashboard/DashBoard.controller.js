sap.ui.define(
  [
    ...
    'sap/m/MessageToast',
    'MyUIWebApp/model/RestModel'
  ],
  function(BaseController, MessageToast, RestModel) {
    'use strict';

    return BaseController.extend('MyUIWebApp.src.pages.dashboard.DashBoard', {
      onInit: function() {
        this.animate();
      },
      animate() {
      ...
      loadMarcas() {
        ...
        model
          ...
          .get()
          .then(() => {
            let item = this.byId('selectMarca')
              .getSelectedItem()
              .getBindingContext('marcas')
              .getObject();
      ...
      loadCarModels(oEvent) {
        ...
        const _dialogModelsAndYear = sap.ui.xmlfragment(
          'MyUIWebApp.src.pages.centercosts.ChooseFromModal',
          this
        );
        _dialogModelsAndYear.addStyleClass(
        ...
        model
          ...
          .catch(err => {
            ctx.showExeption([{ message: 'Ocorreu um erro ao processar a solicitação' }]);
          });
      ...
      filterCars(oEvent) {
        ...
        filterStatus = new sap.ui.model.Filter(
          'fipe_name',
