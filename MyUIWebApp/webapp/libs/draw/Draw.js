sap.ui.define(['sap/ui/core/Control'], function(Control) {
  'use strict';
  return Control.extend('MyUIWebApp.controls.draw.Draw', {
    metadata: {
      properties: {
        width: {
    ...
    init: function() {
      $.sap.require('MyUIWebApp/controls/draw/p5.min.js');
    },
    ...
    renderer: function(oRM, oControl) {
      ...
      let previous = new sap.m.Button({
        ...
        press: () =>
          oControl.fireEvent('onSelectPage', {
    ...
    showAllPage(oEvent, numPages, current) {
      ...
      for (let index = 1; index <= numPages; index++) {
        ...
        let button = new sap.m.Button({
          text: index,
          press: () =>
            that.fireEvent('onSelectPage', {
              selectedPage: index,
              currentPage: currentPage
            })
        });
        if (current == index) button.setType('Accept');
        flex.addItem(button);
      }
